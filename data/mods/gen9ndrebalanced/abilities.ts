...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	curiousmedicine: {
		...
		shortDesc: "Heals 20% HP and cures status on switching out.",
		rating: 4,
		num: 261,
	},
	cursedbody: {
	...
	heroicheart: {
        onModifyMove(move, source, target) {
            if (move.type === 'Water' || move.type === 'Fighting') {
                const waterEffectiveness = this.dex.getEffectiveness('Water', target);
                const fightingEffectiveness = this.dex.getEffectiveness('Fighting', target);

                if (waterEffectiveness > fightingEffectiveness) {
                    move.type = 'Water';
                } else if (fightingEffectiveness > waterEffectiveness) {
                    move.type = 'Fighting';
                }
                this.add('-message', `Heroic Heart changed ${move.name} to ${move.type}-type!`);
            }
        },
        name: "Heroic Heart",
		rating: 5,
